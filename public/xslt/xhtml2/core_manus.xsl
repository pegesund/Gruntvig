...
<xsl:stylesheet
  ...
  <xd:doc type="stylesheet">
    ...
    <xd:cvsId>$Id: core.xsl 6818 2009-10-09 13:49:52Z rahtz $</xd:cvsId>
  ...
  <xsl:template match="tei:add">
  	<xsl:if test="(@type = 'immediate')">
    		<span class="immediate">
      			<xsl:apply-templates/>
    		</span>
  	</xsl:if>
  	<xsl:if test="(@type = 'firstDel')">
    		<span class="firstDel">
      			<xsl:apply-templates/>
    		</span>
  	</xsl:if>
  	<xsl:if test="(@type = 'secondDel')">
    		<span class="secondDel">
      			<xsl:apply-templates/>
    		</span>
  	</xsl:if>
  	<xsl:if test="(@type = 'later')">
		<xsl:text>[</xsl:text>
    		<span class="later" >
      			<xsl:apply-templates/>
    		</span>  
  ...
  <xsl:template match="tei:listBibl">
	...
	<dl class="listBibl">
	  <xsl:for-each select="tei:biblStruct">
	    <xsl:sort select="translate((tei:*/tei:author/tei:surname|
  ...
  <xsl:template name="printNotes">
	...
	<xsl:choose>
      ...
      <xsl:if
	  test="ancestor-or-self::tei:TEI/tei:text/descendant::tei:appNOT-EXISTING">
	
	<div class="notes">
<!-- Petter: Her skrives notene ut -->
	  <div class="noteHeading">
	    <xsl:call-template name="i18n">
	      <xsl:with-param name="word">noteHeading</xsl:with-param>
	    </xsl:call-template>
